...
...
(Use full MIT boilerplate in `LICENSE` file.)

---

## ➤ Next steps I can help with (pick one)
- Produce a ready-to-commit `prompts/primary_prompt.txt` and `system.txt` files.
- Create a sample `examples/sample_output_1.md` using your uploaded FRD (`/mnt/data/Abhishek Singh- TPMUR.pdf`) — I can generate a full How-To guide from that PDF now.
- Write the Rovo-integration script (pseudo-code / YAML) to paste into Confluence.
